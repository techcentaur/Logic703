(* Assignment4: Natural Deduction Proof Style System *)
...
let rec append_delta pt del = match pt with
	...
	| ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (ass3, r1)); Ant (Agree (ass4, r2))]), [cpt1; cpt2; cpt3]) ->
		ElimOr (Con (Agree (union ass1 del, r)), ([Ant (Agree (union ass2 del, Or(p, q))); Ant (Agree (union ass3 del, r1)); Ant (Agree (union ass4 del, r2))]), [append_delta cpt1 del; append_delta cpt2 del; append_delta cpt3 del])
	| IntroOrL (Con(Agree(ass1, Or(p1, q1))), Ant(Agree(ass2, p2)), cpt1) ->
...
let rec replace_2_delta pt del = match pt with 
	...
	| ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (Set(xx::ass3), r1)); Ant (Agree (Set(yy::ass4), r2))]), [cpt1; cpt2; cpt3]) ->
		let x = (union del (Set(xx::[]))) in let y = (union del (Set(yy::[]))) in 
			ElimOr (Con (Agree (del, r)), ([Ant (Agree (del, Or(p, q))); Ant (Agree (x, r1)); Ant (Agree (y, r2))]), [replace_2_delta cpt1 del; replace_2_delta cpt2 x; replace_2_delta cpt3 y])
	| IntroOrL (Con(Agree(ass1, Or(p1, q1))), Ant(Agree(ass2, p2)), cpt1) ->
...
(* Assignment4 - extra functions *)


(* Finding and working on r-pairs *)
...
let rec ____find_rpair_pt____ pt = match pt with
		...
		(match cpt1 with
			...
			| _ -> (let x1 = (____find_rpair_pt____ cpt1) in
				(if List.length x1=0 then 
					( let x2 = (____find_rpair_pt____ cpt2) in
						(if List.length x2=0 then (____find_rpair_pt____ cpt3) else x2)
					)
				else x1)
			)
		)
	...
	| IntroImp (x, y, cpt) -> (____find_rpair_pt____ cpt)
	| IntroAnd (x, y, [cpt1; cpt2]) -> 
		(let x1 = (____find_rpair_pt____ cpt1) in 
			if (List.length x1=0) then (____find_rpair_pt____ cpt2) else x1
		)
	| IntroOrL (x, y, cpt1) -> (____find_rpair_pt____ cpt1)
	| IntroOrR (x, y, cpt1) -> (____find_rpair_pt____ cpt1);;



let rec ____simplify____ pt = match pt with
	| Start (init_g, cpt) -> Start (init_g, ____simplify____ cpt)
	| End(Agree(ass, concl)) -> End(Agree(ass, concl))
	| Int (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), cpt) -> 
		Int (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), (____simplify____ cpt))
	| Class (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), cpt) -> 
		Class (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), ____simplify____ cpt)
	| ElimImp (Con (Agree (ass1, q1)), ([Ant (Agree (ass2, Impl(p, q))); Ant (Agree (ass3, p1))]), [cpt1; cpt2]) ->
		(match cpt1 with
			IntroImp(Con (Agree (xass1, Impl(xp, xq))), Ant (Agree (xass2, xconcl2)), xcpt) ->
				(graft xcpt [cpt2])
			| _ -> 
				ElimImp (Con (Agree (ass1, q1)), ([Ant (Agree (ass2, Impl(p, q))); Ant (Agree (ass3, p1))]), [____simplify____ cpt1; ____simplify____ cpt2])
		)
	| ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (ass3, r1)); Ant (Agree (ass4, r2))]), [cpt1; cpt2; cpt3]) ->
		(match cpt1 with
			IntroOrL (Con(Agree(xass1, Or(xp1, xq1))), Ant(Agree(xass2, xp2)), xcpt1) ->
			...
			| IntroOrR (Con(Agree(xass1, Or(xp1, xq1))), Ant(Agree(xass2, xq2)), xcpt1) ->
				(graft cpt3 [xcpt1])
			| _ ->
		ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (ass3, r1)); Ant (Agree (ass4, r2))]), [____simplify____ cpt1; ____simplify____ cpt2; ____simplify____ cpt3])
		)
	| ElimAndL (Con(Agree(ass1, p)), Ant(Agree(ass2, And(p1, q1))), cpt) ->
		(match cpt with
			IntroAnd (Con (Agree (xass3, And(xp1, xq1))), ([Ant (Agree (xass1, xp)); Ant (Agree (xass2, xq))]), [xcpt1; xcpt2]) ->
				xcpt1
			| _ -> ElimAndL (Con(Agree(ass1, p)), Ant(Agree(ass2, And(p1, q1))), ____simplify____ cpt)
		)
	| ElimAndR (Con(Agree(ass1, p)), Ant(Agree(ass2, And(q1, p1))), cpt) ->
		(match cpt with
			IntroAnd (Con (Agree (xass3, And(xp1, xq1))), ([Ant (Agree (xass1, xp)); Ant (Agree (xass2, xq))]), [xcpt1; xcpt2]) ->
				xcpt2
			| _ -> ElimAndR (Con(Agree(ass1, p)), Ant(Agree(ass2, And(q1, p1))), (____simplify____ cpt))
		)
	| IntroImp (x, y, cpt) -> IntroImp (x, y, (____simplify____ cpt))
	| IntroAnd (x, y, [cpt1; cpt2]) -> IntroAnd (x, y, [____simplify____ cpt1; ____simplify____ cpt2])
	| IntroOrL (x, y, cpt1) -> IntroOrL (x, y, (____simplify____ cpt1))
	| IntroOrR (x, y, cpt1) -> IntroOrR (x, y, (____simplify____ cpt1));;




let rec ____normit____ pt = match pt with
	| Start (init_g, cpt) -> Start (init_g, ____normit____ cpt)
	| End(Agree(ass, concl)) -> End(Agree(ass, concl))
	| Int (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), cpt) -> 
		Int (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), (____normit____ cpt))
	| Class (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), cpt) -> 
		Class (Con (Agree (ass1, concl1)), Ant (Agree (ass2, concl2)), ____normit____ cpt)
	| ElimImp (Con (Agree (ass1, q1)), ([Ant (Agree (ass2, Impl(p, q))); Ant (Agree (ass3, p1))]), [cpt1; cpt2]) ->
		(match cpt1 with
			IntroImp(Con (Agree (xass1, Impl(xp, xq))), Ant (Agree (xass2, xconcl2)), xcpt) ->
				____normit____ (graft xcpt [cpt2])
			| _ -> 
				ElimImp (Con (Agree (ass1, q1)), ([Ant (Agree (ass2, Impl(p, q))); Ant (Agree (ass3, p1))]), [____normit____ cpt1; ____normit____ cpt2])
		)
	| ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (ass3, r1)); Ant (Agree (ass4, r2))]), [cpt1; cpt2; cpt3]) ->
		(match cpt1 with
			IntroOrL (Con(Agree(xass1, Or(xp1, xq1))), Ant(Agree(xass2, xp2)), xcpt1) ->
				____normit____ (graft cpt2 [xcpt1])
			| IntroOrR (Con(Agree(xass1, Or(xp1, xq1))), Ant(Agree(xass2, xq2)), xcpt1) ->
				____normit____ (graft cpt3 [xcpt1])
			| _ ->
		ElimOr (Con (Agree (ass1, r)), ([Ant (Agree (ass2, Or(p, q))); Ant (Agree (ass3, r1)); Ant (Agree (ass4, r2))]), [____normit____ cpt1; ____normit____ cpt2; ____normit____ cpt3])
		)
	| ElimAndL (Con(Agree(ass1, p)), Ant(Agree(ass2, And(p1, q1))), cpt) ->
		(match cpt with
			IntroAnd (Con (Agree (xass3, And(xp1, xq1))), ([Ant (Agree (xass1, xp)); Ant (Agree (xass2, xq))]), [xcpt1; xcpt2]) ->
				____normit____ xcpt1
			| _ -> ElimAndL (Con(Agree(ass1, p)), Ant(Agree(ass2, And(p1, q1))), ____normit____ cpt)
		)
	| ElimAndR (Con(Agree(ass1, p)), Ant(Agree(ass2, And(q1, p1))), cpt) ->
		(match cpt with
			IntroAnd (Con (Agree (xass3, And(xp1, xq1))), ([Ant (Agree (xass1, xp)); Ant (Agree (xass2, xq))]), [xcpt1; xcpt2]) ->
				____normit____ xcpt2
			| _ -> ElimAndR (Con(Agree(ass1, p)), Ant(Agree(ass2, And(q1, p1))), (____normit____ cpt))
		)
	| IntroImp (x, y, cpt) -> IntroImp (x, y, (____normit____ cpt))
	| IntroAnd (x, y, [cpt1; cpt2]) -> IntroAnd (x, y, [____normit____ cpt1; ____normit____ cpt2])
	| IntroOrL (x, y, cpt1) -> IntroOrL (x, y, (____normit____ cpt1))
	| IntroOrR (x, y, cpt1) -> IntroOrR (x, y, (____normit____ cpt1));;
